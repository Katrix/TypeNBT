...
implicit def hListSerializer[Head, Tail <: HList, HeadFoo <: Foo](
    implicit headSerializer: Lazy[FooSerializer.Aux[HeadFoo, Head]],
    tailSerializer: Lazy[FooSerializer.Aux[FooMap, Tail]]) = new FooSerializer {
  override type FooType = FooMap
  override type Repr    = Head :: Tail
  override def write(repr: Repr): FooType =
    FooMap(Map("head" -> headSerializer.value.write(repr.head), "tail" -> tailSerializer.value.write(repr.tail)))
}
